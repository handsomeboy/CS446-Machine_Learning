...
def train(model, mnist_dataset, learning_rate=0.0005, batch_size=16,
    ...
    for step in range(0, num_steps):
        ...
        model.session.run(model.d_opt, feed_dict={model.x_placeholder: batch_x, model.z_placeholder: batch_z})
        model.session.run(model.g_opt, feed_dict={model.z_placeholder: batch_z})
