...
class LinearMRF(object):
    ...
    def get_pairwise_features(self):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        result = np.ones((len(self.pairs),4))
        result[:,1:3] = np.zeros((len(self.pairs),2))
    ...
    def build_training_obj(self, img_features, unary_beliefs, pair_beliefs,
        ...
        for i in range(len(unary_beliefs)):
            obj += tf.reduce_sum(unary_beliefs[i] * unary_potentials[i])
            obj += tf.reduce_sum(pair_beliefs[i] * pairwise_potentials)
            obj -= tf.reduce_sum(img_features * unary_potentials[i])
            #obj -= tf.reduce_sum(self.get_pairwise_beliefs(img_features) * pairwise_potentials)
        obj -= tf.reduce_sum(self.get_pairwise_beliefs(img_features) * pairwise_potentials) * len(unary_beliefs)
    ...
    def inference_itr(self, unary_beliefs, unary_pots, pairwise_pots):
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        nodes = np.arange(unary_beliefs.shape[0])
        np.random.shuffle(nodes)
        for node in nodes:
            score_0 = self.calculate_local_score(node, 0, unary_beliefs, unary_pots, pairwise_pots)
            score_1 = self.calculate_local_score(node, 1, unary_beliefs, unary_pots, pairwise_pots)
            if score_0 > score_1:
    ...
    def calculate_local_score(self, node, assignment, unary_beliefs,
        ...
        for neighbor in self.neighbors[node]:
            if assignment==0 and unary_beliefs[neighbor][1]==0:
                score += pairwise_potentials[self.pair_inds[(node, neighbor)]][0]
            elif assignment==0 and unary_beliefs[neighbor][1]==1:
                score += pairwise_potentials[self.pair_inds[(node, neighbor)]][1]
            elif assignment==1 and unary_beliefs[neighbor][1]==0:
                score += pairwise_potentials[self.pair_inds[(node, neighbor)]][2]
            else:
                score += pairwise_potentials[self.pair_inds[(node, neighbor)]][3]
        return score
    ...
    def check_convergence(self, new_unary_beliefs, old_unary_beliefs,
        ...
        #IMPLEMENT THIS METHOD#
        #######################
        return np.count_nonzero(new_unary_beliefs - old_unary_beliefs)/(2*new_unary_beliefs.shape[0]) < convergence_margin
    ...
    def get_pairwise_beliefs(self, unary_beliefs):
        ...
        for pair in self.pairs:
            if unary_beliefs[pair[0]][1]==0 and unary_beliefs[pair[1]][1]==0:
                result[self.pair_inds[pair]][0] = 1
            elif unary_beliefs[pair[0]][1]==0 and unary_beliefs[pair[1]][1]==1:
                result[self.pair_inds[pair]][1] = 1
            elif unary_beliefs[pair[0]][1]==1 and unary_beliefs[pair[1]][1]==0:
                result[self.pair_inds[pair]][2] = 1
            else:
                result[self.pair_inds[pair]][3] = 1
