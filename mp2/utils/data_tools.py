...
def preprocess_data(dataset,
    ...
    indices = [columns_to_id[feature] for feature in feature_columns]
    indices.sort()
    ...
    for k,v in dataset.items():
        ...
        for idx in indices:
            if idx == 1:
                temp.extend(one_hot_bldg_type(v[idx]))
            else:
                temp.append(int(v[idx]))
        x.append(temp)
        y.append(int(v[-1]))
