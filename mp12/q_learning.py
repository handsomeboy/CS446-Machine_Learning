...
def get_action_index(readout_t, epsilon, t):
    ...
    if t < OBSERVE:
        ...
        prob = [epsilon/ACTIONS] * ACTIONS
        prob.append(1-epsilon)
        action_index = np.random.choice(ACTIONS+1, p=prob)
...
def run_selected_action(a_t, s_t, game_state):
    ...
    """
    next_state, r_t, terminal = game_state.frame_step(a_t)
    ...
    s_t1 = np.dstack((s_t[...,1:], next_state))
...
def trainNetwork(s, readout, sess):
    ...
    if checkpoint and checkpoint.model_checkpoint_path:
        ...
        load = True
        for files in os.listdir("./saved_networks_q_learning"):
            if 'dqn' in files:
                m = re.search(r"(?<=-dqn-)\d+", files)
                last_t = int(m.group(0))
        fd = open( './saved_networks_q_learning/' + 'D_value.data', 'rb')
        D = pickle.load(fd)
    else:
        print("Could not find old network weights")

    # Initialize the epsilon value for the exploration phase.
    epsilon = (INITIAL_EPSILON,FINAL_EPSILON)[load]
    ...
    while t<5000000:
        ...
        # Run the selected action and update the replay memory
        for i in range(0, K):
        ...
        if t % 5000 == 0:
            ...
            fw = open('./saved_networks_q_learning/' + 'D_value.data', 'wb')
            pickle.dump(D, fw)
            fw.close()
            print("Successively saved:", 'saved_networks_q_learning/' + GAME + 'dqn')
